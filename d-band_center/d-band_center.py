...
#Open DOSCAR
dosfile = 'DOSCAR'
doscar = Doscar(dosfile, ispin=1, lmax=3, lorbit=11)  # Calculation settings
atoms = list(range(0, 100))  # if we want to calculate all atoms d-band center and d atom ordinal
#atoms =[98] # here we will set the atom number index and it will print specfic atom number d-band center
#atoms =[81, 85, 86, 90]  #  here we will set the atom number index and it will print specfic atom number d-band center
#Set atoms for integration
orb = 'd'
dos_gold = (doscar.pdos_sum(atoms, spin='up', l=orb))
gold = ((dos_gold))
print (gold)

#Set integrating range of Au
efermi = doscar.efermi - doscar.efermi
energies = doscar.energy - doscar.efermi
emin, emax = energies[0], energies[-1]
erange = (efermi - 7.90, efermi + 5.60)  # Integral energy range
emask = (energies <= erange[-1])

# Calculating d-band center of Au
x  = energies[emask]
y1 = gold[emask]
dbc_au = simps(y1 * x, x) / simps(y1, x)
dbc1 = []
dbc1.append(dbc_au)
print('dbc_au(eV)')
print(dbc1)
